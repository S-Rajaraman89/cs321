...
var w2utils = function () {
    ...
    function isTime(val) {
        ...
        if (tmp[0] == "" || parseInt(tmp[0]) < 0 || parseInt(tmp[0]) > max || !this.isInt(tmp[0])) {
            return false
        }
        if (tmp[1] == "" || parseInt(tmp[1]) < 0 || parseInt(tmp[1]) > 59 || !this.isInt(tmp[1])) {
            return false
        }
        return true
    }
    ...
    function age(timeStr) {
        ...
        if (w2utils.isInt(timeStr))timeStr = Number(timeStr);
        var d1 = new Date(timeStr);
        if (w2utils.isInt(timeStr))d1 = new Date(Number(timeStr));
        var tmp = String(timeStr).split("-");
        if (tmp.length == 3)d1 = new Date(tmp[0], Number(tmp[1]) - 1, tmp[2]);
        var tmp = String(timeStr).split("/");
        if (tmp.length == 3)d1 = new Date(tmp[2], Number(tmp[0]) - 1, tmp[1]);
        if (d1 == "Invalid Time")return"";
        var d2 = new Date;
        var sec = (d2.getTime() - d1.getTime()) / 1e3;
        var amount = "";
        var type = "";
        if (sec < 60) {
    ...
    function date(dateStr) {
        ...
        if (w2utils.isInt(dateStr))dateStr = Number(dateStr);
        var d1 = new Date(dateStr);
        if (w2utils.isInt(dateStr))d1 = new Date(Number(dateStr));
    ...
    function formatDate(dateStr, format) {
        ...
        if (w2utils.isInt(dateStr))dt = new Date(Number(dateStr));
        var tmp = String(dateStr).split("-");
        if (tmp.length == 3)dt = new Date(tmp[0], Number(tmp[1]) - 1, tmp[2]);
        var tmp = String(dateStr).split("/");
        if (tmp.length == 3)dt = new Date(tmp[2], Number(tmp[0]) - 1, tmp[1]);
        if (dt == "Invalid Date")return"";
        var year = dt.getFullYear();
        var month = dt.getMonth();
        var date = dt.getDate();
        return format.toLowerCase().replace("month", w2utils.settings.fullmonths[month]).replace("mon", w2utils.settings.shortmonths[month]).replace(/yyyy/g, year).replace(/yyy/g, year).replace(/yy/g, year > 2e3 ? 100 + parseInt(String(year).substr(2)) : String(year).substr(2)).replace(/(^|[^a-z$])y/g, "$1" + year).replace(/mm/g, (month + 1 < 10 ? "0" : "") + (month + 1)).replace(/dd/g, (date < 10 ? "0" : "") + date).replace(/(^|[^a-z$])m/g, "$1" + (month + 1)).replace(/(^|[^a-z$])d/g, "$1" + date)
    }
    ...
    function formatTime(dateStr, format) {
        ...
        return format.toLowerCase().replace("am", type).replace("pm", type).replace("hh", hour).replace("h24", h24).replace("mm", min).replace("mi", min).replace("ss", sec).replace(/(^|[^a-z$])h/g, "$1" + hour).replace(/(^|[^a-z$])m/g, "$1" + min).replace(/(^|[^a-z$])s/g, "$1" + sec)
    }
    ...
    function stripTags(html) {
        ...
        switch (typeof html) {
            case"number":
                break;
            case"string":
                html = $.trim(String(html).replace(/(<([^>]+)>)/gi, ""));
                break;
            case"object":
    ...
    function transition(div_old, div_new, type, callBack) {
        ...
        setTimeout(function () {
            ...
            if (div_new) {
                $(div_new).css({opacity: "1", "-webkit-transition": "", "-moz-transition": "", "-ms-transition": "", "-o-transition": "", "-webkit-transform": "", "-moz-transform": "", "-ms-transform": "", "-o-transform": "", "-webkit-backface-visibility": "", "-moz-backface-visibility": "", "-ms-backface-visibility": "", "-o-backface-visibility": ""})
            }
            if (div_old) {
                $(div_old).css({opacity: "1", "-webkit-transition": "", "-moz-transition": "", "-ms-transition": "", "-o-transition": "", "-webkit-transform": "", "-moz-transform": "", "-ms-transform": "", "-o-transform": "", "-webkit-backface-visibility": "", "-moz-backface-visibility": "", "-ms-backface-visibility": "", "-o-backface-visibility": ""});
                if (div_old.parentNode)$(div_old.parentNode).css({"-webkit-perspective": "", "-moz-perspective": "", "-ms-perspective": "", "-o-perspective": ""})
            }
    ...
    function lock(box, msg, showSpinner) {
        if (!msg && msg != 0)msg = "";
        w2utils.unlock(box);
        $(box).find(">:first-child").before('<div class="w2ui-lock"></div>' + '<div class="w2ui-lock-msg"></div>');
        setTimeout(function () {
            ...
            setTimeout(function () {
                ...
                var top = ($(box).height() * .9 - w2utils.getSize(mess, "height")) / 2;
                lock.css({opacity: lock.data("old_opacity"), left: "0px", top: "0px", width: "100%", height: "100%"});
                ...
                mess.html(msg).css({opacity: mess.data("old_opacity"), left: left + "px", top: top + "px"})
            }, 10)
        }, 10);
        $().w2tag()
    }
    ...
    function getSize(el, type) {
        var bwidth = {left: parseInt($(el).css("border-left-width")) || 0, right: parseInt($(el).css("border-right-width")) || 0, top: parseInt($(el).css("border-top-width")) || 0, bottom: parseInt($(el).css("border-bottom-width")) || 0};
        var mwidth = {left: parseInt($(el).css("margin-left")) || 0, right: parseInt($(el).css("margin-right")) || 0, top: parseInt($(el).css("margin-top")) || 0, bottom: parseInt($(el).css("margin-bottom")) || 0};
        var pwidth = {left: parseInt($(el).css("padding-left")) || 0, right: parseInt($(el).css("padding-right")) || 0, top: parseInt($(el).css("padding-top")) || 0, bottom: parseInt($(el).css("padding-bottom")) || 0};
        switch (type) {
            case"top":
    ...
    function locale(locale) {
        ...
        $.ajax({url: locale, type: "GET", dataType: "JSON", async: false, cache: false, success: function (data, status, xhr) {
    ...
    function scrollBarSize() {
        ...
        if (String(navigator.userAgent).indexOf("MSIE") >= 0)tmp.scrollBarSize = tmp.scrollBarSize / 2;
        return tmp.scrollBarSize
    }
}();
w2utils.event = {on: function (eventData, handler) {
    ...
    var eventData = $.extend({type: null, phase: "before", target: null, isStopped: false, isCancelled: false}, eventData, {preventDefault: function () {
    ...
    for (var h = this.handlers.length - 1; h >= 0; h--) {
        ...
        if ((item.event.type == eventData.type || item.event.type == "*") && (item.event.target == eventData.target || item.event.target == null) && (item.event.execute == eventData.phase || item.event.execute == "*" || item.event.phase == "*")) {
            ...
            if (args.length == 2) {
                item.handler.call(this, eventData.target, eventData)
            } else {
    ...
    if (eventData.phase == "before" && typeof this[funName] == "function") {
        ...
        if (tmp)args = tmp[1].split(/\s*,\s*/);
        if (args.length == 2) {
            fun.call(this, eventData.target, eventData)
        } else {
            fun.call(this, eventData)
        }
        if (eventData.isStopped === true || eventData.stop === true)return eventData
    }
    if (typeof eventData.object != "undefined" && eventData.object != null && eventData.phase == "before" && typeof eventData.object[funName] == "function") {
...
(function () {
            ...
            if (text == "" || text == null || typeof text == "undefined") {
                ...
                var timer = setInterval(function () {
                    ...
                    if ($("#w2ui-tag-" + tagID).data("position") != $(el).offset().left + el.offsetWidth + "x" + $(el).offset().top) {
                        $("#w2ui-tag-" + tagID).css({"-webkit-transition": ".2s", "-moz-transition": ".2s", "-ms-transition": ".2s", "-o-transition": ".2s", left: $(el).offset().left + el.offsetWidth + "px", top: $(el).offset().top + "px"}).data("position", $(el).offset().left + el.offsetWidth + "x" + $(el).offset().top)
                    }
                }, 100);
                setTimeout(function () {
                    if (!$(el).offset())return;
                    $("#w2ui-tag-" + tagID).css({opacity: "1", left: $(el).offset().left + el.offsetWidth + "px", top: $(el).offset().top + "px"}).html('<div style="margin-top: -2px 0px 0px -2px; white-space: nowrap;"> <div class="w2ui-tag-body">' + text + "</div> </div>").data("text", text).data("taged-el", el).data("options", options).data("position", $(el).offset().left + el.offsetWidth + "x" + $(el).offset().top).data("timer", timer);
    ...
    $.fn.w2overlay = function (html, options) {
        ...
        $("body").append('<div id="w2ui-overlay" class="w2ui-reset w2ui-overlay ' + ($(this).parents(".w2ui-popup").length > 0 ? "w2ui-overlay-popup" : "") + '">' + "	<div></div>" + "</div>");
        var obj = this;
        ...
        div.css({display: "none", left: $(obj).offset().left + options.left + "px", top: $(obj).offset().top + w2utils.getSize($(obj), "height") + 3 + options.top + "px", "min-width": options.width ? options.width : "auto", "min-height": options.height ? options.height : "auto"}).fadeIn("fast").data("position", $(obj).offset().left + "x" + ($(obj).offset().top + obj.offsetHeight)).on("click", function (event) {
    ...
    $.fn.w2menu = function (menu, options) {
        ...
        function getMenuHTML() {
            ...
            for (var f = 0; f < menu.length; f++) {
                ...
                menu_html += "<tr onmouseover=\"$(this).addClass('w2ui-selected');\" onmouseout=\"$(this).removeClass('w2ui-selected');\" " + "		onclick=\"$(document).click(); $.fn.w2menuHandler(event, '" + f + "');\">" + img + "	<td>" + mitem.text + "</td>" + "</tr>"
            }
            menu_html += "</table>";
            return menu_html
        }
...
(function () {
    var w2grid = function (options) {
        ...
        this.postData = {};
        this.toolbar = {};
        this.show = {header: false, toolbar: false, footer: false, columnHeaders: true, lineNumbers: false, expandColumn: false, selectColumn: false, emptyRecords: true, toolbarReload: true, toolbarColumns: true, toolbarSearch: true, toolbarAdd: false, toolbarEdit: false, toolbarDelete: false, toolbarSave: false, selectionBorder: true, recordTitles: true};
        this.autoLoad = true;
        this.fixedBody = true;
        ...
        this.onResize = null;
        this.onDestroy = null;
        this.last = {field: "all", caption: w2utils.lang("All Fields"), logic: "OR", search: "", searchIds: [], multi: false, scrollTop: 0, scrollLeft: 0, selected: [], sortData: null, sortCount: 0, xhr: null, range_start: null, range_end: null, sel_ind: null, sel_col: null, sel_type: null};
        this.isIOS = navigator.userAgent.toLowerCase().indexOf("iphone") != -1 || navigator.userAgent.toLowerCase().indexOf("ipod") != -1 || navigator.userAgent.toLowerCase().indexOf("ipad") != -1 ? true : false;
    ...
    w2grid.prototype = {msgDelete: "Are you sure you want to delete selected records?", msgNotJSON: "Returned data is not in valid JSON format.", msgRefresh: "Refreshing...", buttons: {reload: {type: "button", id: "reload", img: "icon-reload", hint: "Reload data in the list"}, columns: {type: "drop", id: "column-on-off", img: "icon-columns", hint: "Show/hide columns", arrow: false, html: ""}, search: {type: "html", id: "search", html: '<div class="w2ui-icon icon-search-down w2ui-search-down" title="' + "Select Search Field" + '" ' + "onclick=\"var obj = w2ui[$(this).parents('div.w2ui-grid').attr('name')]; obj.searchShowFields(this);\"></div>"}, "search-go": {type: "check", id: "search-advanced", caption: "Search...", hint: "Open Search Fields"}, add: {type: "button", id: "add", caption: "Add New", hint: "Add new record", img: "icon-add"}, edit: {type: "button", id: "edit", caption: "Edit", hint: "Edit selected record", img: "icon-edit", disabled: true}, "delete": {type: "button", id: "delete", caption: "Delete", hint: "Delete selected records", img: "icon-delete", disabled: true}, save: {type: "button", id: "save", caption: "Save", hint: "Save changed records", img: "icon-save"}}, add: function (record) {
        ...
        return recs
    }, set: function (recid, record, noRefresh) {
        ...
        if (recid == null) {
            for (var r in this.records) {
                $.extend(true, this.records[r], record)
            }
        ...
        if (silent !== true)setTimeout(function () {
            obj.status("Sorting took " + time / 1e3 + " sec")
        }, 10);
        return time
            ...
            for (var r in this.records) {
                ...
                for (var s in this.searchData) {
                    ...
                    switch (sdata.operator) {
                        case"is":
                            if (rec[search.field] == sdata.value)fl++;
                            if (search.type == "date") {
                                var tmp = new Date(Number(val1));
                                val1 = new Date(tmp.getFullYear(), tmp.getMonth(), tmp.getDate()).getTime();
                                val2 = Number(val2);
                                var val3 = Number(val1) + 864e5;
                        ...
                        case"between":
                            ...
                            if (search.type == "date") {
                                var tmp = new Date(Number(val3));
                                val3 = new Date(tmp.getFullYear(), tmp.getMonth(), tmp.getDate()).getTime();
                                var val3 = Number(val3) + 864e5;
                        ...
                        case"contains":
                            ...
                            break;
                        case"ends with":
                            if (val1.indexOf(val2) == val1.length - val2.length)fl++;
                            break
                    }
                }
        ...
        time = (new Date).getTime() - time;
        if (silent !== true)setTimeout(function () {
            obj.status("Search took " + time / 1e3 + " sec")
        }, 10);
        return time
        ...
        for (var r in ranges) {
            ...
            if (first) {
                var rg = {name: ranges[r].name, range: [
                    {recid: first.recid, column: first.column},
                    {recid: last.recid, column: last.column}
        ...
        for (var r in this.ranges) {
            ...
            if (td1.length > 0 && td2.length > 0) {
                $("#grid_" + this.name + "_" + rg.name).css({left: td1.position().left - 1 + rec.scrollLeft() + "px", top: td1.position().top - 1 + rec.scrollTop() + "px", width: td2.position().left - td1.position().left + td2.width() + 3 + "px", height: td2.position().top - td1.position().top + td2.height() + 3 + "px"})
            }
        }
        ...
        function mouseStart(event) {
            ...
            obj.last.move = {type: "expand", x: event.screenX, y: event.screenY, divX: 0, divY: 0, recid: sel[0].recid, column: sel[0].column, originalRange: [
                {recid: sel[0].recid, column: sel[0].column},
                {recid: sel[sel.length - 1].recid, column: sel[sel.length - 1].column}
            ], newRange: [
                {recid: sel[0].recid, column: sel[0].column},
                {recid: sel[sel.length - 1].recid, column: sel[sel.length - 1].column}
            ]};
        ...
        function mouseMove(event) {
            ...
            mv.newRange = [
                {recid: mv.recid, column: mv.column},
                {recid: recid, column: column}
            ];
            ...
            if (eventData.isCancelled === true) {
                ...
                obj.addRange({name: "grid-selection-expand", range: eventData.newRange, style: "background-color: rgba(100,100,100,0.1); border: 2px dotted rgba(100,100,100,0.5);"})
            }
        }
            ...
            if (this.selectType == "row") {
                ...
                if (!w2utils.isInt(col)) {
                    ...
                    for (var c in this.columns) {
                        ...
                        cols.push({recid: recid, column: parseInt(c)})
                    }
                ...
                record.selectedColumns.sort(function (a, b) {
                    return a - b
                });
        ...
        if ($("#grid_" + this.name + "_records").find(".grid_select_check[type=checkbox]").length != 0 && $("#grid_" + this.name + "_records").find(".grid_select_check[type=checkbox]").length == $("#grid_" + this.name + "_records").find(".grid_select_check[type=checkbox]:checked").length) {
            $("#grid_" + this.name + "_check_all").prop("checked", true)
        } else {
        ...
        if (arguments.length == 0) {
            ...
            for (var s in this.searches) {
                ...
                if (value1 != "" && value1 != null || typeof value2 != "undefined" && value2 != "") {
                    var tmp = {field: search.field, type: search.type, operator: operator};
                    if (operator == "between") {
            ...
            if (typeof value != "undefined") {
                if (field.toLowerCase() == "all") {
                    if (this.searches.length > 0) {
                        for (var s in this.searches) {
                            var search = this.searches[s];
                            if (search.type == "text" || search.type == "int" && w2utils.isInt(value) || search.type == "float" && w2utils.isFloat(value) || search.type == "money" && w2utils.isMoney(value) || search.type == "hex" && w2utils.isHex(value) || search.type == "date" && w2utils.isDate(value) || search.type == "alphaNumeric" && w2utils.isAlphaNumeric(value)) {
                                var tmp = {field: search.field, type: search.type, operator: search.type == "text" ? "contains" : "is", value: value};
                                searchData.push(tmp)
                            }
                            if (search.type == "int" && String(value).indexOf("-") != -1) {
                                var t = String(value).split("-");
                                var tmp = {field: search.field, type: search.type, operator: "between", value: [t[0], t[1]]};
                                searchData.push(tmp)
                            }
                        }
                    } else {
                        for (var c in this.columns) {
                            var tmp = {field: this.columns[c].field, type: "text", operator: "contains", value: value};
                            searchData.push(tmp)
                        }
                    }
                } else {
                    ...
                    if (value != "") {
                        ...
                        var tmp = {field: search.field, type: search.type, operator: op, value: val};
                        searchData.push(tmp)
                    }
                }
            }
        ...
        var eventData = this.trigger({phase: "before", type: "search", target: this.name, searchData: searchData, searchField: field ? field : "multi", searchValue: value ? value : "multi"});
        if (eventData.isCancelled === true)return;
        ...
        var obj = this;
        $("#tb_" + this.name + "_toolbar_item_search-advanced").w2overlay(this.getSearchesHTML(), {left: -10, "class": "w2ui-grid-searches", onShow: function () {
        ...
        this.last.xhr = $.ajax({type: xhr_type, url: url, data: String($.param(eventData.postData, false)).replace(/%5B/g, "[").replace(/%5D/g, "]"), dataType: "text", complete: function (xhr, status) {
        ...
        if (status != "error") {
            if (typeof responseText != "undefined" && responseText != "") {
                ...
                if (typeof data == "undefined") {
                    data = {status: "error", message: this.msgNotJSON, responseText: responseText}
                }
        ...
        var eventData = obj.trigger({phase: "before", type: "editField", target: obj.name, recid: recid, column: column, value: value, index: index, originalEvent: event});
        if (eventData.isCancelled === true)return;
        ...
        el.addClass("w2ui-editable").html('<input id="grid_' + obj.name + "_edit_" + recid + "_" + column + '" value="' + val + '" type="text"  ' + '	style="outline: none; ' + addStyle + edit.style + '" field="' + col.field + '" recid="' + recid + '" column="' + column + '" ' + edit.inTag + ">" + edit.outTag);
        el.find("input").w2field(edit.type, edit).on("click",function (event) {
            event.stopPropagation()
        }).on("blur",function (event) {
            if (obj.parseField(rec, col.field) != this.value) {
                var eventData2 = obj.trigger({phase: "before", type: "change", target: obj.name, input_id: this.id, recid: recid, column: column, value_new: this.value, value_previous: rec.changes ? rec.changes[col.field] : obj.parseField(rec, col.field), value_original: obj.parseField(rec, col.field)});
        ...
        var tmp = [187, 189];
        for (var i = 48; i <= 90; i++)tmp.push(i);
        ...
        var addClass = "";
        $("#grid_" + this.name + "_rec_" + id).after('<tr id="grid_' + this.name + "_rec_" + id + '_expanded_row" class="w2ui-expanded-row ' + addClass + '">' + (this.show.lineNumbers ? '<td class="w2ui-col-number"></td>' : "") + '	<td class="w2ui-grid-data w2ui-expanded1" colspan="' + tmp + '"><div style="display: none"></div></td>' + '	<td colspan="100" class="w2ui-expanded2">' + '		<div id="grid_' + this.name + "_rec_" + id + '_expanded" style="opacity: 0"></div>' + "	</td>" + "</tr>");
        var eventData = this.trigger({phase: "before", type: "expand", target: this.name, recid: recid, box_id: "grid_" + this.name + "_rec_" + id + "_expanded", ready: ready});
        if (eventData.isCancelled === true) {
        ...
        var id = w2utils.escapeId(recid);
        if ($("#grid_" + this.name + "_rec_" + id + "_expanded_row").length == 0)return false;
        var eventData = this.trigger({phase: "before", type: "collapse", target: this.name, recid: recid, box_id: "grid_" + this.name + "_rec_" + id + "_expanded"});
        if (eventData.isCancelled === true)return false;
        ...
        return true
    }, sort: function (field, direction, multiField) {
        ...
        function mouseStart(event) {
            ...
            obj.last.move = {x: event.screenX, y: event.screenY, divX: 0, divY: 0, recid: $(event.target).parents("tr").attr("recid"), column: event.target.tagName == "TD" ? $(event.target).attr("col") : $(event.target).parents("td").attr("col"), type: "select", start: true};
        ...
        function mouseMove(event) {
            ...
            if (Math.abs(obj.last.move.divX) <= 1 && Math.abs(obj.last.move.divY) <= 1)return;
            ...
            if (obj.selectType != "row") {
                ...
                if (obj.multiSelect) {
                    var sel = obj.getSelection();
                    for (var ns in newSel)if (sel.indexOf(newSel[ns]) == -1)obj.select(newSel[ns]);
                    for (var s in sel)if (newSel.indexOf(sel[s]) == -1)obj.unselect(sel[s])
                }
            }
        }
        ...
        var obj = this;
        var col_html = '<div class="w2ui-col-on-off">' + "<table>";
        for (var c in this.columns) {
            var col = this.columns[c];
            col_html += "<tr>" + "<td>" + '	<input id="grid_' + this.name + "_column_" + c + '_check" type="checkbox" tabIndex="-1" ' + (col.hidden ? "" : "checked") + "		onclick=\"w2ui['" + obj.name + "'].columnOnOff(this, event, '" + col.field + "');\">" + "</td>" + "<td>" + '	<label for="grid_' + this.name + "_column_" + c + '_check">' + (this.columns[c].caption == "" ? "- column " + (c + 1) + " -" : this.columns[c].caption) + "</label>" + "</td>" + "</tr>"
        }
        col_html += '<tr><td colspan="2"><div style="border-top: 1px solid #ddd;"></div></td></tr>';
        var url = typeof this.url != "object" ? this.url : this.url.get;
        if (url) {
            col_html += '<tr><td colspan="2" style="padding: 0px">' + '	<div style="cursor: pointer; padding: 2px 8px; cursor: default">' + "		" + w2utils.lang("Skip") + ' <input type="text" style="width: 45px" value="' + this.offset + '" ' + "				onchange=\"w2ui['" + obj.name + "'].columnOnOff(this, event, 'skip', this.value);\"> " + w2utils.lang("Records") + "	</div>" + "</td></tr>"
        }
        col_html += '<tr><td colspan="2" onclick="w2ui[\'' + obj.name + "'].columnOnOff(this, event, 'line-numbers');\">" + '	<div style="cursor: pointer; padding: 4px 8px; cursor: default">' + w2utils.lang("Toggle Line Numbers") + "</div>" + "</td></tr>" + '<tr><td colspan="2" onclick="w2ui[\'' + obj.name + "'].columnOnOff(this, event, 'resize');\">" + '	<div style="cursor: pointer; padding: 4px 8px; cursor: default">' + w2utils.lang("Reset Column Size") + "</div>" + "</td></tr>";
        col_html += "</table></div>";
        ...
        $(this.box).find(".w2ui-resizer").off("click").on("click",function (event) {
            ...
            obj.resizing = true;
            obj.last.tmp = {x: event.screenX, y: event.screenY, gx: event.screenX, gy: event.screenY, col: parseInt($(this).attr("name"))};
            if (event.stopPropagation)event.stopPropagation(); else event.cancelBubble = true;
            if (event.preventDefault)event.preventDefault();
            for (var c in obj.columns) {
            ...
            var td = $(el).parent();
            $(el).css({height: "25px", "margin-left": td.width() - 3 + "px"})
        })
        ...
        if (this.show.toolbar) {
            toolbar.css({top: 0 + (this.show.header ? w2utils.getSize(header, "height") : 0) + "px", left: "0px", right: "0px"})
        }
        if (this.show.footer) {
            footer.css({bottom: "0px", left: "0px", right: "0px"})
        }
        if (this.summary.length > 0) {
            summary.css({bottom: 0 + (this.show.footer ? w2utils.getSize(footer, "height") : 0) + "px", left: "0px", right: "0px"})
        }
        ...
        if (bodyOverflowX || bodyOverflowY) {
            ...
            records.css({top: (this.columnGroups.length > 0 && this.show.columns ? 1 : 0) + w2utils.getSize(columns, "height") + "px", "-webkit-overflow-scrolling": "touch", "overflow-x": bodyOverflowX ? "auto" : "hidden", "overflow-y": bodyOverflowY ? "auto" : "hidden"})
        } else {
        ...
        if (columns.find("> table > tbody > tr").length == 3) {
            columns.find("> table > tbody > tr:nth-child(1) td").html("").css({height: "0px", border: "0px", padding: "0px", margin: "0px"})
        }
        ...
        for (var i = 0; i < this.searches.length; i++) {
            ...
            switch (s.type) {
                ...
                case"money":
                case"date":
                    html += '<input rel="search" type="text" size="12" id="grid_' + this.name + "_field_" + i + '" name="' + s.field + '" ' + s.inTag + ">" + '<span id="grid_' + this.name + "_range_" + i + '" style="display: none">' + '&nbsp;-&nbsp;&nbsp;<input rel="search" type="text" size="12" id="grid_' + this.name + "_field2_" + i + '" name="' + s.field + '" ' + s.inTag + ">" + "</span>";
                    break;
                case"list":
        ...
        html += "<tr>" + '	<td colspan="4" class="actions">' + "		<div>" + '		<input type="button" onclick="obj = w2ui[\'' + this.name + '\']; if (obj) { obj.searchReset(); }" value="' + w2utils.lang("Reset") + '">' + '		<input type="button" onclick="obj = w2ui[\'' + this.name + '\']; if (obj) { obj.search(); }" value="' + w2utils.lang("Search") + '">' + "		</div>" + "	</td>" + "</tr></table>";
        return html
        ...
        function getGroups() {
            ...
            for (var i = 0; i < obj.columnGroups.length; i++) {
                ...
                if (colg.master === true) {
                    ...
                    html += '<td class="w2ui-head ' + sortStyle + '" col="' + ii + '" rowspan="2" colspan="' + (colg.span + (i == obj.columnGroups.length - 1 ? 1 : 0)) + '" ' + (col.sortable ? "onclick=\"w2ui['" + obj.name + "'].columnClick('" + col.field + "', event);\"" : "") + ">" + resizer + '	<div class="w2ui-col-group ' + sortStyle + '">' + (col.caption == "" ? "&nbsp;" : col.caption) + "	</div>" + "</td>"
                } else {
        ...
        function getColumns(master) {
            ...
            for (var i = 0; i < obj.columns.length; i++) {
                ...
                if (colg["master"] !== true || master) {
                    ...
                    html += '<td col="' + i + '" class="w2ui-head ' + sortStyle + '" ' + (col.sortable ? "onclick=\"w2ui['" + obj.name + "'].columnClick('" + col.field + "', event);\"" : "") + ">" + resizer + '	<div class="' + sortStyle + '">' + (col.caption == "" ? "&nbsp;" : col.caption) + "	</div>" + "</td>"
                }
            }
        ...
        html += '<tr id="grid_' + this.name + '_rec_bottom" line="bottom" style="height: ' + (this.buffered - limit) * this.recordHeight + 'px">' + '	<td colspan="200"></td>' + "</tr>" + '<tr id="grid_' + this.name + '_rec_more" style="display: none">' + '	<td colspan="200" class="w2ui-load-more"></td>' + "</tr>" + "</table>";
        ...
        var url = typeof this.url != "object" ? this.url : this.url.get;
        $("#grid_" + this.name + "_footer .w2ui-footer-right").html(w2utils.formatNumber(this.offset + t1) + "-" + w2utils.formatNumber(this.offset + t2) + " " + w2utils.lang("of") + " " + w2utils.formatNumber(this.total) + (url ? " (" + w2utils.lang("buffered") + " " + w2utils.formatNumber(this.buffered) + (this.offset > 0 ? ", skip " + w2utils.formatNumber(this.offset) : "") + ")" : ""));
        ...
        if (this.show.expandColumn) {
            ...
            rec_html += '<td id="grid_' + this.name + "_cell_" + ind + '_expand" class="w2ui-grid-data w2ui-col-expand">' + (summary !== true ? '	<div ondblclick="if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;" ' + "			onclick=\"w2ui['" + this.name + "'].toggle('" + record.recid + "', event); " + '				if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">' + "		" + tmp_img + " </div>" : "") + "</td>"
        }
        var col_ind = 0;
        while (true) {
        ...
        if (typeof col.render != "undefined") {
            ...
            if (typeof col.render == "string") {
                ...
                if ($.inArray(tmp[0], ["number", "int", "float", "money", "percent"]) != -1) {
                    ...
                    if (tmp[0] == "money") {
                        tmp[1] = 2;
                        prefix = w2utils.settings.currencySymbol
                    }
                    if (tmp[0] == "percent") {
                        suffix = "%";
                        if (tmp[1] !== "0")tmp[1] = 1
        ...
        return"<div>" + '	<div class="w2ui-footer-left"></div>' + '	<div class="w2ui-footer-right">' + this.buffered + "</div>" + '	<div class="w2ui-footer-center"></div>' + "</div>"
    }, status: function (msg) {
...
(function () {
    ...
    $.fn.w2layout = function (method) {
        ...
        function initTabs(object, panel, tabs) {
            ...
            $().w2destroy(object.name + "_" + panel + "_tabs");
        ...
        function initToolbar(object, panel, toolbar) {
            ...
            $().w2destroy(object.name + "_" + panel + "_toolbar");
            pan.toolbar = $().w2toolbar($.extend({}, toolbar, {owner: object, name: object.name + "_" + panel + "_toolbar"}));
            pan.show.toolbar = true;
            return true
        }
    };
    w2layout.prototype = {panel: {type: null, size: 100, minSize: 20, hidden: false, resizable: false, overflow: "auto", style: "", content: "", tabs: null, toolbar: null, width: null, height: null, show: {toolbar: false, tabs: false}, onRefresh: null, onShow: null, onHide: null}, html: function (panel, data, transition) {
        ...
        if (this.get(panel) != null) {
            $.get(url, function (data, status, xhr) {
                ...
                if (onLoad)onLoad();
                if (window.navigator.userAgent.indexOf("MSIE"))setTimeout(function () {
                    obj.resize()
                }, 100)
            });
            return true
        ...
        $(obj.box).find(" > div > .w2ui-panel").css({"-webkit-transition": ".35s", "-moz-transition": ".35s", "-ms-transition": ".35s", "-o-transition": ".35s"});
        setTimeout(function () {
            obj.set(panel, {size: size})
        }, 1);
        setTimeout(function () {
            $(obj.box).find(" > div > .w2ui-panel").css({"-webkit-transition": "0s", "-moz-transition": "0s", "-ms-transition": "0s", "-o-transition": "0s"});
            obj.resize()
        }, 500);
        return true
        ...
        if (immediate === true) {
            ...
            $(obj.box).find(" > div > .w2ui-panel").css({"-webkit-transition": ".2s", "-moz-transition": ".2s", "-ms-transition": ".2s", "-o-transition": ".2s"});
            setTimeout(function () {
                obj.resize()
            }, 1);
            setTimeout(function () {
        ...
        for (var t in tmp) {
            ...
            var html = '<div id="layout_' + obj.name + "_panel_" + tmp[t] + '" class="w2ui-panel">' + '	<div class="w2ui-panel-tabs"></div>' + '	<div class="w2ui-panel-toolbar"></div>' + '	<div class="w2ui-panel-content"></div>' + "</div>" + '<div id="layout_' + obj.name + "_resizer_" + tmp[t] + '" class="w2ui-resizer"></div>';
        ...
        return(new Date).getTime() - time;
        function initEvents() {
            obj.tmp.events = {resize: function (event) {
                w2ui[obj.name].resize()
            }, resizeStart: resizeStart, mousemove: resizeMove, mouseup: resizeStop};
        ...
        function resizeStart(type, evnt) {
            ...
            obj.tmp.resize = {type: type, x: evnt.screenX, y: evnt.screenY, div_x: 0, div_y: 0, value: 0};
        ...
        function resizeStop(evnt) {
            ...
            if (obj.tmp.div_x != 0 || obj.tmp.resize.div_y != 0) {
                ...
                switch (obj.tmp.resize.type) {
                    ...
                    case"preview":
                        var ns = parseInt(panel.sizeCalculated) - obj.tmp.resize.div_y;
                        var nd = (ptop && !ptop.hidden ? ptop.sizeCalculated : 0) + (pbottom && !pbottom.hidden ? pbottom.sizeCalculated : 0);
                        break;
                    case"left":
        ...
        if (panel != null && typeof panel != "undefined") {
            ...
            obj.resize();
            for (var p in this.panels) {
                obj.refresh(this.panels[p].type)
            }
        }
        obj.trigger($.extend(eventData, {phase: "after"}));
        return(new Date).getTime() - time
        ...
        $(this.box).find(" > div").css({width: width + "px", height: height + "px"});
        var obj = this;
        ...
        if (ptop != null && ptop.hidden != true) {
            ...
            $("#layout_" + this.name + "_panel_top").css({display: "block", left: l + "px", top: t + "px", width: w + "px", height: h + "px"}).show();
            ptop.width = w;
            ptop.height = h;
            if (ptop.resizable) {
                t = ptop.sizeCalculated - (this.padding == 0 ? this.resizer : 0);
                h = this.resizer > this.padding ? this.resizer : this.padding;
                $("#layout_" + this.name + "_resizer_top").show().css({display: "block", left: l + "px", top: t + "px", width: w + "px", height: h + "px", cursor: "ns-resize"}).bind("mousedown", function (event) {
        ...
        if (pleft != null && pleft.hidden != true) {
            ...
            var h = height - (stop ? ptop.sizeCalculated + this.padding : 0) - (sbottom ? pbottom.sizeCalculated + this.padding : 0);
            var e = $("#layout_" + this.name + "_panel_left");
            if (window.navigator.userAgent.indexOf("MSIE") > 0 && e.length > 0 && e[0].clientHeight < e[0].scrollHeight)w += 17;
            $("#layout_" + this.name + "_panel_left").css({display: "block", left: l + "px", top: t + "px", width: w + "px", height: h + "px"}).show();
            pleft.width = w;
            pleft.height = h;
            if (pleft.resizable) {
                l = pleft.sizeCalculated - (this.padding == 0 ? this.resizer : 0);
                w = this.resizer > this.padding ? this.resizer : this.padding;
                $("#layout_" + this.name + "_resizer_left").show().css({display: "block", left: l + "px", top: t + "px", width: w + "px", height: h + "px", cursor: "ew-resize"}).bind("mousedown", function (event) {
        ...
        if (pright != null && pright.hidden != true) {
            ...
            var h = height - (stop ? ptop.sizeCalculated + this.padding : 0) - (sbottom ? pbottom.sizeCalculated + this.padding : 0);
            $("#layout_" + this.name + "_panel_right").css({display: "block", left: l + "px", top: t + "px", width: w + "px", height: h + "px"}).show();
            pright.width = w;
            pright.height = h;
            if (pright.resizable) {
                l = l - this.padding;
                w = this.resizer > this.padding ? this.resizer : this.padding;
                $("#layout_" + this.name + "_resizer_right").show().css({display: "block", left: l + "px", top: t + "px", width: w + "px", height: h + "px", cursor: "ew-resize"}).bind("mousedown", function (event) {
        ...
        if (pbottom != null && pbottom.hidden != true) {
            ...
            $("#layout_" + this.name + "_panel_bottom").css({display: "block", left: l + "px", top: t + "px", width: w + "px", height: h + "px"}).show();
            pbottom.width = w;
            pbottom.height = h;
            if (pbottom.resizable) {
                t = t - (this.padding == 0 ? 0 : this.padding);
                h = this.resizer > this.padding ? this.resizer : this.padding;
                $("#layout_" + this.name + "_resizer_bottom").show().css({display: "block", left: l + "px", top: t + "px", width: w + "px", height: h + "px", cursor: "ns-resize"}).bind("mousedown", function (event) {
        ...
        var t = 0 + (stop ? ptop.sizeCalculated + this.padding : 0);
        var w = width - (sleft ? pleft.sizeCalculated + this.padding : 0) - (sright ? pright.sizeCalculated + this.padding : 0);
        var h = height - (stop ? ptop.sizeCalculated + this.padding : 0) - (sbottom ? pbottom.sizeCalculated + this.padding : 0) - (sprev ? pprev.sizeCalculated + this.padding : 0);
        var e = $("#layout_" + this.name + "_panel_main");
        if (window.navigator.userAgent.indexOf("MSIE") > 0 && e.length > 0 && e[0].clientHeight < e[0].scrollHeight)w += 17;
        $("#layout_" + this.name + "_panel_main").css({display: "block", left: l + "px", top: t + "px", width: w + "px", height: h + "px"});
        pmain.width = w;
        pmain.height = h;
        if (pprev != null && pprev.hidden != true) {
            ...
            if (window.navigator.userAgent.indexOf("MSIE") > 0 && e.length > 0 && e[0].clientHeight < e[0].scrollHeight)w += 17;
            $("#layout_" + this.name + "_panel_preview").css({display: "block", left: l + "px", top: t + "px", width: w + "px", height: h + "px"}).show();
            pprev.width = w;
            pprev.height = h;
            if (pprev.resizable) {
                t = t - (this.padding == 0 ? 0 : this.padding);
                h = this.resizer > this.padding ? this.resizer : this.padding;
                $("#layout_" + this.name + "_resizer_preview").show().css({display: "block", left: l + "px", top: t + "px", width: w + "px", height: h + "px", cursor: "ns-resize"}).bind("mousedown", function (event) {
...
(function () {
    ...
    w2popup = {defaults: {title: "", body: "", buttons: "", style: "", color: "#000", opacity: .4, speed: .3, modal: false, maximized: false, keyboard: true, width: 500, height: 300, showClose: true, showMax: false, transition: null}, handlers: [], onOpen: null, onClose: null, onMax: null, onMin: null, onKeydown: null, open: function (options) {
        var obj = this;
        ...
        var top = (parseInt(height) - parseInt(options.height)) / 2 * .6;
        ...
        if ($("#w2ui-popup").length == 0) {
            ...
            var msg = '<div id="w2ui-popup" class="w2ui-popup" style="' + "width: " + parseInt(options.width) + "px; height: " + parseInt(options.height) + "px; opacity: 0; " + "-webkit-transform: scale(0.8); -moz-transform: scale(0.8); -ms-transform: scale(0.8); -o-transform: scale(0.8); " + "left: " + left + "px; top: " + top + 'px;">';
            if (options.title != "") {
                msg += '<div class="w2ui-msg-title">' + (options.showClose ? '<div class="w2ui-msg-button w2ui-msg-close" onclick="w2popup.close(); ' + 'if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">Close</div>' : "") + (options.showMax ? '<div class="w2ui-msg-button w2ui-msg-max" onclick="w2popup.toggle()">Max</div>' : "") + options.title + "</div>"
            }
            ...
            setTimeout(function () {
                ...
                $("#w2ui-popup").css({"-webkit-transition": options.speed + "s opacity, " + options.speed + "s -webkit-transform", "-webkit-transform": "scale(1)", "-moz-transition": options.speed + "s opacity, " + options.speed + "s -moz-transform", "-moz-transform": "scale(1)", "-ms-transition": options.speed + "s opacity, " + options.speed + "s -ms-transform", "-ms-transform": "scale(1)", "-o-transition": options.speed + "s opacity, " + options.speed + "s -o-transform", "-o-transform": "scale(1)", opacity: "1"})
            }, 1);
            setTimeout(function () {
                $("#w2ui-popup").css({"-webkit-transform": "", "-moz-transform": "", "-ms-transform": "", "-o-transform": ""});
                obj.trigger($.extend(eventData, {phase: "after"}))
            }, options.speed * 1e3)
        } else {
            ...
            $("#w2ui-popup .w2ui-msg-title").html((options.showClose ? '<div class="w2ui-msg-button w2ui-msg-close" onclick="w2popup.close()">Close</div>' : "") + (options.showMax ? '<div class="w2ui-msg-button w2ui-msg-max" onclick="w2popup.max()">Max</div>' : "") + options.title);
        ...
        $("#w2ui-popup .w2ui-msg-body").on("mousemove",function (event) {
            mvMove(event)
        }).on("mouseup", function (event) {
            mvStop(event)
        });
        $("#w2ui-lock").on("mousemove",function (event) {
            mvMove(event)
        }).on("mouseup", function (event) {
            mvStop(event)
        });
        ...
        if (eventData.isCancelled === true)return;
        $("#w2ui-popup, #w2ui-panel").css({"-webkit-transition": options.speed + "s opacity, " + options.speed + "s -webkit-transform", "-webkit-transform": "scale(0.9)", "-moz-transition": options.speed + "s opacity, " + options.speed + "s -moz-transform", "-moz-transform": "scale(0.9)", "-ms-transition": options.speed + "s opacity, " + options.speed + "s -ms-transform", "-ms-transform": "scale(0.9)", "-o-transition": options.speed + "s opacity, " + options.speed + "s -o-transform", "-o-transform": "scale(0.9)", opacity: "0"});
        ...
        $("#w2ui-popup .w2ui-popup-message").css({"-webkit-transform": display == "none" ? "translateY(-" + options.height + "px)" : "translateY(0px)", "-moz-transform": display == "none" ? "translateY(-" + options.height + "px)" : "translateY(0px)", "-ms-transform": display == "none" ? "translateY(-" + options.height + "px)" : "translateY(0px)", "-o-transform": display == "none" ? "translateY(-" + options.height + "px)" : "translateY(0px)"});
        if (display == "none") {
            $("#w2ui-popup .w2ui-popup-message").show().html(options.html);
            setTimeout(function () {
                $("#w2ui-popup .w2ui-popup-message").css({"-webkit-transition": "0s", "-moz-transition": "0s", "-ms-transition": "0s", "-o-transition": "0s", "z-Index": 1500});
                w2popup.lock();
        ...
        setTimeout(function () {
            $("#w2ui-popup .w2ui-popup-message").css({"-webkit-transform": display == "none" ? "translateY(0px)" : "translateY(-" + options.height + "px)", "-moz-transform": display == "none" ? "translateY(0px)" : "translateY(-" + options.height + "px)", "-ms-transform": display == "none" ? "translateY(0px)" : "translateY(-" + options.height + "px)", "-o-transform": display == "none" ? "translateY(0px)" : "translateY(-" + options.height + "px)"})
        }, 1)
    }, lock: function (msg, showSpinner) {
        ...
        setTimeout(function () {
            $("#w2ui-lock").css({"-webkit-transition": options.speed + "s opacity", "-moz-transition": options.speed + "s opacity", "-ms-transition": options.speed + "s opacity", "-o-transition": options.speed + "s opacity", opacity: options.opacity})
        }, 1);
        if (options.modal == true) {
            $("#w2ui-lock").on("mousedown", function () {
                $("#w2ui-lock").css({"-webkit-transition": ".1s", "-moz-transition": ".1s", "-ms-transition": ".1s", "-o-transition": ".1s", opacity: "0.6"});
                if (window.getSelection)window.getSelection().removeAllRanges()
            });
            $("#w2ui-lock").on("mouseup", function () {
                setTimeout(function () {
                    $("#w2ui-lock").css({"-webkit-transition": ".1s", "-moz-transition": ".1s", "-ms-transition": ".1s", "-o-transition": ".1s", opacity: options.opacity})
                }, 100);
                if (window.getSelection)window.getSelection().removeAllRanges()
            })
        } else {
        ...
        if ($("#w2ui-lock").length == 0)return false;
        var options = $.extend({}, $("#w2ui-popup").data("options"), options);
        $("#w2ui-lock").css({"-webkit-transition": options.speed + "s opacity", "-moz-transition": options.speed + "s opacity", "-ms-transition": options.speed + "s opacity", "-o-transition": options.speed + "s opacity", opacity: 0});
        setTimeout(function () {
            $("#w2ui-lock").remove()
        }, options.speed * 1e3);
        return true
        ...
        var top = (parseInt($(window).height()) - parseInt(height)) / 2 * .8;
        var left = (parseInt($(window).width()) - parseInt(width)) / 2;
        $("#w2ui-popup").css({"-webkit-transition": options.speed + "s width, " + options.speed + "s height, " + options.speed + "s left, " + options.speed + "s top", "-moz-transition": options.speed + "s width, " + options.speed + "s height, " + options.speed + "s left, " + options.speed + "s top", "-ms-transition": options.speed + "s width, " + options.speed + "s height, " + options.speed + "s left, " + options.speed + "s top", "-o-transition": options.speed + "s width, " + options.speed + "s height, " + options.speed + "s left, " + options.speed + "s top", top: top, left: left, width: width, height: height});
...
var w2alert = function (msg, title, callBack) {
    ...
    if ($("#w2ui-popup").length > 0) {
        w2popup.message({width: 400, height: 150, html: '<div style="position: absolute; top: 0px; left: 0px; right: 0px; bottom: 40px; overflow: auto">' + '		<div class="w2ui-centered"><div style="font-size: 13px;">' + msg + "</div></div>" + "</div>" + '<div style="position: absolute; bottom: 7px; left: 0px; right: 0px; text-align: center; padding: 5px">' + '		<input type="button" value="Ok" onclick="w2popup.message();" class="w2ui-popup-button">' + "</div>", onClose: function () {
            if (typeof callBack == "function")callBack()
        }})
    } else {
        w2popup.open({width: 450, height: 200, showMax: false, title: title, body: '<div class="w2ui-centered"><div style="font-size: 13px;">' + msg + "</div></div>", buttons: '<input type="button" value="' + w2utils.lang("Ok") + '" class="w2ui-popup-button" onclick="w2popup.close();">', onClose: function () {
            if (typeof callBack == "function")callBack()
        }})
    }
};
var w2confirm = function (msg, title, callBack) {
    ...
    if ($("#w2ui-popup").length > 0) {
        w2popup.message({width: 400, height: 150, html: '<div style="position: absolute; top: 0px; left: 0px; right: 0px; bottom: 40px; overflow: auto">' + '		<div class="w2ui-centered"><div style="font-size: 13px;">' + msg + "</div></div>" + "</div>" + '<div style="position: absolute; bottom: 7px; left: 0px; right: 0px; text-align: center; padding: 5px">' + '		<input id="No" type="button" value="' + w2utils.lang("No") + '" class="w2ui-popup-button">' + '		<input id="Yes" type="button" value="' + w2utils.lang("Yes") + '" class="w2ui-popup-button">' + "</div>", onOpen: function () {
        ...
        w2popup.open({width: 450, height: 200, title: title, modal: true, showClose: false, body: '<div class="w2ui-centered"><div style="font-size: 13px;">' + msg + "</div></div>", buttons: '<input id="No" type="button" value="' + w2utils.lang("No") + '" class="w2ui-popup-button">' + '<input id="Yes" type="button" value="' + w2utils.lang("Yes") + '" class="w2ui-popup-button">', onOpen: function (event) {
...
(function () {
    ...
    w2tabs.prototype = {tab: {id: null, text: "", hidden: false, disabled: false, closable: false, hint: "", onClick: null, onRefresh: null, onClose: null}, add: function (tab) {
        ...
        if (jq_el.length == 0) {
            ...
            if (tab.disabled) {
                jq_el.css({opacity: "0.2", "-moz-opacity": "0.2", "-webkit-opacity": "0.2", "-o-opacity": "0.2", filter: "alpha(opacity=20)"})
            } else {
                jq_el.css({opacity: "1", "-moz-opacity": "1", "-webkit-opacity": "1", "-o-opacity": "1", filter: "alpha(opacity=100)"})
            }
        }
        this.trigger($.extend(eventData, {phase: "after"}));
        return(new Date).getTime() - time
        ...
        if (eventData.isCancelled === true)return false;
        var obj = this;
        $(this.box).find("#tabs_" + this.name + "_tab_" + w2utils.escapeId(tab.id)).css({"-webkit-transition": ".2s", "-moz-transition": "2s", "-ms-transition": ".2s", "-o-transition": ".2s", opacity: "0"});
        setTimeout(function () {
...
(function () {
    var w2toolbar = function (options) {
        this.box = null, this.name = null, this.items = [], this.right = "", this.onClick = null, this.onRender = null, this.onRefresh = null, this.onResize = null, this.onDestroy = null;
    ...
    w2toolbar.prototype = {item: {id: null, type: "button", text: "", html: "", img: null, icon: null, hidden: false, disabled: false, checked: false, arrow: true, hint: "", group: null, items: null, onClick: null}, add: function (items) {
        ...
        switch (item.type) {
            ...
            case"drop":
                ...
                html += '<table cellpadding="0" cellspacing="0" title="' + item.hint + '" class="w2ui-button ' + (item.checked ? "checked" : "") + '" ' + "       onclick     = \"var el=w2ui['" + this.name + "']; if (el) el.click('" + item.id + "', event);\" " + '       onmouseover = "' + (!item.disabled ? "$(this).addClass('over');" : "") + '"' + '       onmouseout  = "' + (!item.disabled ? "$(this).removeClass('over');" : "") + '"' + '       onmousedown = "' + (!item.disabled ? "$(this).addClass('down');" : "") + '"' + '       onmouseup   = "' + (!item.disabled ? "$(this).removeClass('down');" : "") + '"' + ">" + "<tr><td>" + '  <table cellpadding="1" cellspacing="0">' + "  <tr>" + img + (item.text != "" ? '<td class="w2ui-tb-caption" nowrap>' + item.text + "</td>" : "") + ((item.type == "drop" || item.type == "menu") && item.arrow !== false ? '<td class="w2ui-tb-down" nowrap>&nbsp;&nbsp;&nbsp;</td>' : "") + "  </tr></table>" + "</td></tr></table>";
                break;
            case"break":
        ...
        var it = this.get(id);
        if (it && !it.disabled) {
            var eventData = this.trigger({phase: "before", type: "click", target: typeof id != "undefined" ? id : this.name, item: this.get(id), subItem: typeof menu_index != "undefined" && this.get(id) ? this.get(id).items[menu_index] : null, originalEvent: event});
            if (eventData.isCancelled === true)return false;
            this.trigger($.extend(eventData, {phase: "after"}))
        }
    }, click: function (id, event) {
        ...
        var it = this.get(id);
        if (it && !it.disabled) {
            var eventData = this.trigger({phase: "before", type: "click", target: typeof id != "undefined" ? id : this.name, item: this.get(id), originalEvent: event});
            if (eventData.isCancelled === true)return false;
            ...
            if (it.type == "drop" || it.type == "menu") {
                if (it.checked) {
                    ...
                    setTimeout(function () {
                        ...
                        if (it.type == "menu") {
                            el.w2menu(it.items, $.extend({left: (el.width() - 50) / 2, top: 3}, it.overlay, {select: function (item, event, index) {
                                obj.menuClick(it.id, index, event)
                            }}))
                        }
...
(function () {
    ...
    w2sidebar.prototype = {node: {id: null, text: "", count: "", img: null, icon: null, nodes: [], style: "", selected: false, expanded: false, hidden: false, disabled: false, group: false, plus: false, onClick: null, onDblClick: null, onContextMenu: null, onExpand: null, onCollapse: null, parent: null, sidebar: null}, add: function (parent, nodes) {
        if (arguments.length == 1) {
            nodes = arguments[0];
            parent = this
        }
        ...
        for (var o in nodes) {
            ...
            if (typeof nd != "undefined" && nd.length > 0) {
        ...
        return false
    }, get: function (parent, id, returnIndex) {
        ...
        if (nd.disabled || nd.group)return;
        ...
        setTimeout(function () {
            ...
            if (obj.menu.length > 0) {
                $(obj.box).find("#node_" + w2utils.escapeId(id)).w2menu(obj.menu, {left: (event ? event.offsetX || event.pageX : 50) - 25, select: function (item, event, index) {
        ...
        $(this.box).find("> div").css({width: $(this.box).width() + "px", height: $(this.box).height() + "px"});
        if ($(this.box).length > 0)$(this.box)[0].style.cssText += this.style;
        if (this.topHTML != "") {
        ...
        function getNodeHTML(nd) {
            ...
            if (nd.group) {
                html = '<div class="w2ui-node-group"  id="node_' + nd.id + '"' + "		onclick=\"w2ui['" + obj.name + "'].toggle('" + nd.id + "'); " + "				 var sp=$(this).find('span:nth-child(1)'); if (sp.html() == '" + w2utils.lang("Hide") + "') sp.html('" + w2utils.lang("Show") + "'); else sp.html('" + w2utils.lang("Hide") + "');\"" + "		onmouseout=\"$(this).find('span:nth-child(1)').css('color', 'transparent')\" " + "		onmouseover=\"$(this).find('span:nth-child(1)').css('color', 'inherit')\">" + "	<span>" + (!nd.hidden && nd.expanded ? w2utils.lang("Hide") : w2utils.lang("Show")) + "</span>" + "	<span>" + nd.text + "</span>" + "</div>" + '<div class="w2ui-node-sub" id="node_' + nd.id + '_sub" style="' + nd.style + ";" + (!nd.hidden && nd.expanded ? "" : "display: none;") + '"></div>'
            } else {
...
(function ($) {
    ...
    $.fn.w2field = function (method) {
        if (w2field[method]) {
            ...
            return w2field.init.apply(this, [
                {type: method}
            ])
        } else {
    ...
    $.extend(w2field, {init: function (options) {
        var obj = w2field;
        return $(this).each(function (field, index) {
            ...
            switch (tp) {
                case"clear":
                    $(this).off("focus").off("blur").off("keypress").off("keydown").off("change").removeData();
                    if ($(this).prev().hasClass("w2ui-list")) {
                        $(this).prev().remove();
                        $(this).removeAttr("tabindex").css("border-color", "").show()
                    }
                    if ($(this).prev().hasClass("w2ui-upload")) {
                        $(this).prev().remove();
                        $(this).removeAttr("tabindex").css("border-color", "").show()
                    }
                    if ($(this).prev().hasClass("w2ui-field-helper")) {
                        $(this).css("padding-left", $(this).css("padding-top"));
                        $(this).prev().remove()
                    }
                    if ($(this).next().hasClass("w2ui-field-helper")) {
                ...
                case"hex":
                    var el = this;
                    var defaults = {min: null, max: null, arrows: false, keyboard: true, suffix: "", prefix: ""};
                    options = $.extend({}, defaults, options);
                    if (["text", "alphanumeric", "hex"].indexOf(tp) != -1) {
                        options.arrows = false;
                        options.keyboard = false
                    }
                    $(this).data("options", options).on("keypress",function (event) {
                        ...
                        var v = $(el).val();
                        var cancel = false;
                        if (options.min != null && v != "" && v < options.min) {
                            $(el).val(options.min).change();
                            cancel = true
                        }
                        if (options.max != null && v != "" && v > options.max) {
                            $(el).val(options.max).change();
                            cancel = true
                        }
                        if (cancel) {
                            event.stopPropagation();
                    ...
                    if (options.prefix != "") {
                        $(this).before('<div class="w2ui-field-helper">' + options.prefix + "</div>");
                        var helper = $(this).prev();
                        helper.css({color: $(this).css("color"), "font-family": $(this).css("font-family"), "font-size": $(this).css("font-size"), "padding-top": $(this).css("padding-top"), "padding-bottom": $(this).css("padding-bottom"), "padding-left": $(this).css("padding-left"), "padding-right": 0, "margin-top": parseInt($(this).css("margin-top")) + 1 + "px", "margin-bottom": parseInt($(this).css("margin-bottom")) + 1 + "px", "margin-left": 0, "margin-right": 0}).on("click", function () {
                            $(this).next().focus()
                        });
                    ...
                    if (options.arrows != "") {
                        $(this).after('<div class="w2ui-field-helper" style="border: 1px solid transparent">&nbsp;' + '	<div class="w2ui-field-up" type="up">' + '		<div class="arrow-up" type="up"></div>' + "	</div>" + '	<div class="w2ui-field-down" type="down">' + '		<div class="arrow-down" type="down"></div>' + "	</div>" + '	<div style="position: absolute; height: 1px; border-top: 1px solid red"></div>' + "</div>");
                        var height = w2utils.getSize(this, "height");
                        var helper = $(this).next();
                        helper.css({color: $(this).css("color"), "font-family": $(this).css("font-family"), "font-size": $(this).css("font-size"), height: $(this).height() + parseInt($(this).css("padding-top")) + parseInt($(this).css("padding-bottom")) + "px", padding: "0px", "margin-top": parseInt($(this).css("margin-top")) + 1 + "px", "margin-bottom": "0px", "border-left": "1px solid silver"}).css("margin-left", "-" + (helper.width() + parseInt($(this).css("margin-right")) + 12) + "px").on("mousedown", function (event) {
                            var btn = this;
                            var evt = event;
                            $("body").on("mouseup", tmp);
                            ...
                            function update(notimer) {
                                $(el).focus().trigger($.Event("keydown"), {keyCode: $(evt.target).attr("type") == "up" ? 38 : 40});
                    ...
                    if (options.suffix != "") {
                        $(this).after('<div class="w2ui-field-helper">' + options.suffix + "</div>");
                        var helper = $(this).next();
                        helper.css({color: $(this).css("color"), "font-family": $(this).css("font-family"), "font-size": $(this).css("font-size"), "padding-top": $(this).css("padding-top"), "padding-bottom": $(this).css("padding-bottom"), "padding-left": "3px", "padding-right": $(this).css("padding-right"), "margin-top": parseInt($(this).css("margin-top")) + 1 + "px", "margin-bottom": parseInt($(this).css("margin-bottom")) + 1 + "px"}).on("click", function () {
                            $(this).prev().focus()
                        });
                ...
                case"date":
                    var obj = this;
                    var defaults = {format: w2utils.settings.date_format, start: "", end: "", blocked: {}, colored: {}};
                    options = $.extend({}, defaults, options);
                    $(this).css({transition: "none", "-webkit-transition": "none", "-moz-transition": "none", "-ms-transition": "none", "-o-transition": "none"}).data("options", options).on("focus",function () {
                        ...
                        $("body").append('<div id="global_calendar_div" style="top: ' + (top + parseInt(obj.offsetHeight)) + "px; left: " + left + 'px;" ' + ' class="w2ui-reset w2ui-calendar" ' + ' onmousedown="' + "		if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true; " + '		if (event.preventDefault) event.preventDefault(); else return false;">' + "</div>");
                        $("#global_calendar_div").html($().w2field("calendar_get", obj.value, options)).css({left: left + "px", top: top + "px"}).data("el", obj).show();
                        ...
                        var mtimer = setInterval(function () {
                            ...
                            if ($("#global_calendar_div").data("position") != $(obj).offset().left + "x" + ($(obj).offset().top + obj.offsetHeight)) {
                                $("#global_calendar_div").css({"-webkit-transition": ".2s", left: left + "px", top: $(obj).offset().top + obj.offsetHeight + "px"}).data("position", $(obj).offset().left + "x" + ($(obj).offset().top + obj.offsetHeight))
                            }
                ...
                case"color":
                    var obj = this;
                    var defaults = {prefix: "#", suffix: '<div style="margin-top: 1px; height: 12px; width: 12px;"></div>'};
                    options = $.extend({}, defaults, options);
                    $(this).attr("maxlength", 6).on("focus",function () {
                        var top = parseFloat($(obj).offset().top) + parseFloat(obj.offsetHeight);
                        var left = parseFloat($(obj).offset().left);
                        clearInterval($(obj).data("mtimer"));
                        $("#global_color_div").remove();
                        $("body").append('<div id="global_color_div" style="top: ' + (top + parseInt(obj.offsetHeight)) + "px; left: " + left + 'px;" ' + ' class="w2ui-reset w2ui-calendar" ' + ' onmousedown="' + "		if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true; " + '		if (event.preventDefault) event.preventDefault(); else return false;">' + "</div>");
                        $("#global_color_div").html($().w2field("getColorHTML", obj.value)).css({left: left + "px", top: top + "px"}).data("el", obj).show();
                        var max = $(window).width() + $(document).scrollLeft() - 1;
                        ...
                        var mtimer = setInterval(function () {
                            ...
                            if ($("#global_color_div").data("position") != $(obj).offset().left + "x" + ($(obj).offset().top + obj.offsetHeight)) {
                                $("#global_color_div").css({"-webkit-transition": ".2s", left: left + "px", top: $(obj).offset().top + obj.offsetHeight + "px"}).data("position", $(obj).offset().left + "x" + ($(obj).offset().top + obj.offsetHeight))
                            }
                            if ($(obj).length == 0 || $(obj).offset().left == 0 && $(obj).offset().top == 0) {
                                clearInterval(mtimer);
                                $("#global_color_div").remove();
                ...
                case"list":
                    ...
                    var defaults = {url: "", items: [], value: null, showNone: true};
                    var obj = this;
                    ...
                    if (settings.url != "") {
                        $.ajax({type: "GET", dataType: "text", url: settings.url, complete: function (xhr, status) {
                            if (status == "success") {
                ...
                case"enum":
                    ...
                    var defaults = {url: "", items: [], selected: [], max: 0, maxHeight: 172, showAll: false, match: "begins with", render: null, maxCache: 500, onShow: null, onHide: null, onAdd: null, onRemove: null, onItemOver: null, onItemOut: null, onItemClick: null};
                    var obj = this;
                    var settings = $.extend({}, defaults, options);
                    ...
                    $(this).css({padding: "0px", "border-color": "transparent", "background-color": "transparent", outline: "none"});
                    ...
                    this.show = function () {
                        ...
                        div.css({display: "block", left: $(obj).offset().left + "px", top: $(obj).offset().top + obj.offsetHeight + 3 + "px"}).width(w2utils.getSize(obj, "width")).data("position", $(obj).offset().left + "x" + ($(obj).offset().top + obj.offsetHeight));
                        ...
                        var monitor = function () {
                            ...
                            if (div.data("position") != $(obj).offset().left + "x" + ($(obj).offset().top + obj.offsetHeight)) {
                                div.css({"-webkit-transition": ".2s", left: $(obj).offset().left + "px", top: $(obj).offset().top + obj.offsetHeight + 3 + "px"}).data("position", $(obj).offset().left + "x" + ($(obj).offset().top + obj.offsetHeight));
                                setTimeout(function () {
                    ...
                    this.refresh = function () {
                        ...
                        var margin = "margin-top: " + $(this).css("margin-top") + "; " + "margin-bottom: " + $(this).css("margin-bottom") + "; " + "margin-left: " + $(this).css("margin-left") + "; " + "margin-right: " + $(this).css("margin-right") + "; " + "width: " + (w2utils.getSize(this, "width") - parseInt($(this).css("margin-left")) - parseInt($(this).css("margin-right"))) + "px; ";
                        var html = '<div class="w2ui-list" style="' + margin + ';">' + '<div style="padding: 0px; margin: 0px; display: inline-block"><ul>';
                        var selected = $(this).data("selected");
                        for (var s in selected) {
                            html += '<li style="' + ($(this).data("last_del") == s ? "opacity: 0.5" : "") + '">' + '	<div title="' + w2utils.lang("Remove") + '" index="' + s + '">&nbsp;&nbsp;</div>' + selected[s].text + "</li>"
                        }
                        html += '<li style="padding-left: 0px; padding-right: 0px" class="nomouse">' + '	<input type="text" ' + ($(this).attr("readonly") ? "readonly" : "") + ' style="background-color: transparent">' + "</li>" + "</ul></div>" + "</div>";
                        $(this).before(html);
                        var div = $(this).prev()[0];
                        $(this).data("div", div);
                        ...
                        }).find("input").on("focus",function (event) {
                    ...
                    this.resize = function () {
                        var settings = $(this).data("settings");
                        var div = $(this).prev();
                        var cntHeight = $(div).find(">div").height();
                        ...
                        if (div.length > 0)div[0].scrollTop = 1e3;
                ...
                case"upload":
                    ...
                    var defaults = {url: "", base64: true, hint: w2utils.lang("Attach files by dragging and dropping or Click to Select"), max: 0, maxSize: 0, maxFileSize: 0, onAdd: null, onRemove: null, onItemClick: null, onItemDblClick: null, onItemOver: null, onItemOut: null, onProgress: null, onComplete: null};
                    var obj = this;
                    var settings = $.extend({}, defaults, options);
                    if (settings.base64 === true) {
                        if (settings.maxSize == 0)settings.maxSize = 20 * 1024 * 1024;
                        if (settings.maxFileSize == 0)settings.maxFileSize = 20 * 1024 * 1024
                    }
                    ...
                    this.refresh = function () {
                        ...
                        for (var s in selected) {
                            ...
                            $(div).find(".file-list").append('<li id="file-' + cnt + '">' + '	<div class="file-delete" onmouseover="event.stopPropagation();">&nbsp;&nbsp;</div>' + '	<span class="file-name">' + file.name + "</span>" + '	<span class="file-size"> - ' + w2utils.size(file.size) + "</span>" + "</li>");
                            var li = $(div).find(".file-list #file-" + cnt);
                            var previewHTML = "";
                            if (/image/i.test(file.type)) {
                                previewHTML = '<div style="padding: 2px;">' + '	<img src="##FILE##" style="max-width: 300px;" ' + '		onload="var w = $(this).width(); var h = $(this).height(); ' + "			if (w < 300 & h < 300) return; " + "			if (w >= h && w > 300) $(this).width(300);" + '			if (w < h && h > 300) $(this).height(300);"' + "		onerror=\"this.style.display = 'none'\"" + "	>" + "</div>"
                            }
                            var td1 = 'style="padding: 3px; text-align: right; color: #777;"';
                            var td2 = 'style="padding: 3px"';
                            previewHTML += '<div style="padding: 5px;">' + '	<table cellpadding="2">' + "	<tr><td " + td1 + ">Name:</td><td " + td2 + ">" + file.name + "</td></tr>" + "	<tr><td " + td1 + ">Size:</td><td " + td2 + ">" + w2utils.size(file.size) + "</td></tr>" + "	<tr><td " + td1 + ">Type:</td><td " + td2 + ">" + '		<span style="width: 200px; display: block-inline; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">' + file.type + "</span>" + "	</td></tr>" + "	<tr><td " + td1 + ">Modified:</td><td " + td2 + ">" + w2utils.date(file.modified) + "</td></tr>" + "	</table>" + "</div>";
                            li.data("file", file).on("click",function (event) {
        ...
        var margin = "margin-top: " + $(obj).css("margin-top") + "; " + "margin-bottom: " + $(obj).css("margin-bottom") + "; " + "margin-left: " + $(obj).css("margin-left") + "; " + "margin-right: " + $(obj).css("margin-right") + "; " + "width: " + (w2utils.getSize(obj, "width") - parseInt($(obj).css("margin-left")) - parseInt($(obj).css("margin-right"))) + "px; " + "height: " + (w2utils.getSize(obj, "height") - parseInt($(obj).css("margin-top")) - parseInt($(obj).css("margin-bottom"))) + "px; ";
        var html = '<div style="' + margin + '" class="w2ui-upload">' + "	<span>" + settings.hint + "</span>" + '	<ul class="file-list"></ul>' + '	<input class="file-input" type="file" name="attachment" multiple style="display: none">' + "</div>";
        $(obj).css({display1: "none", "border-color": "transparent"}).before(html);
        $(obj).data("div", $(obj).prev()[0]);
        var div = $(obj).data("div");
        $(div).find(".file-input").off("change").on("change", function () {
        ...
        }).off("dragenter").on("dragenter",function (event) {
        ...
        var settings = $(obj).data("settings");
        var selected = $(obj).data("selected");
        var newItem = {name: file.name, type: file.type, modified: file.lastModifiedDate, size: file.size, content: null};
        var size = 0;
        var cnt = 0;
        for (var s in selected) {
        ...
        if (settings.url != "" && (items.length == 0 && settings.last_total != 0 || search.length > settings.last_search_len && settings.last_total > settings.maxCache || search.length < settings.last_search_match && search.length != settings.last_search_len)) {
            var match = false;
            if (settings.last_total < settings.maxCache)match = true;
            $.ajax({type: "GET", dataType: "text", url: settings.url, data: {search: search, max: settings.maxCache}, complete: function (xhr, status) {
                settings.last_total = 0;
                if (status == "success") {
                    var data = $.parseJSON(xhr.responseText);
                    if (match == false && data.total < settings.maxCache) {
                        settings.last_search_match = search.length
                    }
        ...
        for (var a in items) {
            ...
            if (match) {
                ...
                if (txt !== false) {
                    ...
                    ihtml += '\n<li index="' + a + '" value="' + id + '" ' + "  onmouseover=\"$(this).parent().find('li').removeClass('selected'); $(this).addClass('selected'); \"" + '	class="' + (i % 2 ? "w2ui-item-even" : "w2ui-item-odd") + (i == $(obj).data("last_index") ? " selected" : "") + '">' + txt + "</li>";
        ...
        if (parseInt(div.height()) > max_height) {
            div.css({height: max_height - 5 + "px", overflow: "show"});
            $(div).find(".w2ui-items-list").css({height: max_height - 15 + "px", overflow: "auto"})
        }
        $(div).off("mousedown").on("mousedown", function (event) {
        ...
        $(obj).prev().find("li > input").val(search).css("max-width", $(div).width() - 25 + "px").width((search.length + 2) * 6 + "px").focus().on("click",function (event) {
            if (event.stopPropagation)event.stopPropagation(); else event.cancelBubble = true
            ...
            setTimeout(function () {
                ...
                if ($.inArray(event.keyCode, [16, 91, 37, 39]) == -1) {
        ...
        var html = '<table cellpadding="0" cellspacing="0"><tr>' + "<td>" + $().w2field("calendar_month", dt.getMonth() + 1, dt.getFullYear(), options) + "</td>" + "</tr></table>";
        return html
        ...
        var html = '<div class="w2ui-calendar-title" onclick="event.stopPropagation()">' + "	<div class=\"w2ui-calendar-previous\" onclick=\"$().w2field('calendar_previous', '" + month + "/" + year + "')\"> <- </div>" + "	<div class=\"w2ui-calendar-next\" onclick=\"$().w2field('calendar_next', '" + month + "/" + year + "')\"> -> </div> " + months[month - 1] + ", " + year + "</div>" + '<table class="w2ui-calendar-days" onclick="event.stopPropagation()" cellspacing="0">' + '	<tr class="w2ui-day-title">' + dayTitle + "</tr>" + "	<tr>";
        var day = 1;
        for (var ci = 1; ci < 43; ci++) {
            ...
            var bgcol = "";
            var blocked = "";
            if (options.colored)if (options.colored[dt] != undefined) {
            ...
            if (noSelect === false) {
                html += "onclick=\"var el = $('#global_calendar_div.w2ui-calendar').data('el'); " + "	$(el).val('" + w2utils.formatDate(dt, options.format) + "').trigger('change').trigger('blur'); " + "	 if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;" + "	 if (event.preventDefault) event.preventDefault(); else return false;" + '"'
            }
            html += ">" + dspDay + "</td>";
        ...
        for (var i = 0; i < 8; i++) {
            html += "<tr>";
            for (var j = 0; j < 8; j++) {
                html += "<td>" + "	<div onclick=\"var el = $('#global_color_div').data('el');" + "			$(el).val($(this).attr('name')).change(); " + "			$('#global_color_div').hide()\" " + '		style="background-color: #' + colors[i][j] + ';" name="' + colors[i][j] + '">' + "		" + (color == colors[i][j] ? "&#149;" : "&nbsp;") + "	</div>" + "</td>"
            }
            html += "</tr>";
...
(function () {
    ...
    w2form.prototype = {get: function (field, returnIndex) {
        ...
        this.last.xhr = $.ajax({type: "GET", url: url, data: String($.param(eventData.postData, false)).replace(/%5B/g, "[").replace(/%5D/g, "]"), dataType: "text", complete: function (xhr, status) {
            obj.unlock();
            ...
            if (status != "error") {
                if (typeof responseText != "undefined" && responseText != "") {
                    ...
                    if (typeof data == "undefined") {
                        data = {status: "error", message: obj.msgNotJSON, responseText: responseText}
                    }
                    if (data["status"] == "error") {
                        obj.error(data["message"])
                    } else {
        ...
        setTimeout(function () {
            ...
            obj.last.xhr = $.ajax({type: w2utils.settings.RESTfull ? obj.recid == 0 ? "POST" : "PUT" : "POST", url: url, data: String($.param(eventData.postData, false)).replace(/%5B/g, "[").replace(/%5D/g, "]"), dataType: "text", xhr: function () {
        ...
        for (var f in this.fields) {
            ...
            html += '\n   <div class="w2ui-label ' + (typeof field.html.span != "undefined" ? "w2ui-span" + field.html.span : "") + '">' + field.html.caption + ":</div>" + '\n   <div class="w2ui-field ' + (typeof field.html.span != "undefined" ? "w2ui-span" + field.html.span : "") + '">' + input + field.html.text + "</div>";
        ...
        function resizeElements() {
            ...
            tabs.css("top", (obj.header != "" ? w2utils.getSize(header, "height") : 0) + (obj.toolbar.items.length > 0 ? w2utils.getSize(toolbar, "height") : 0));
            page.css("top", (obj.header != "" ? w2utils.getSize(header, "height") : 0) + (obj.toolbar.items.length > 0 ? w2utils.getSize(toolbar, "height") + 5 : 0) + (obj.tabs.tabs.length > 0 ? w2utils.getSize(tabs, "height") + 5 : 0));
        ...
        for (var f in this.fields) {
            ...
            $(field.el).w2field("clear").off("change").on("change", function () {
                ...
                if ((field.type == "enum" || field.type == "upload") && $(this).data("selected")) {
                    ...
                    var value_new = [];
                    var value_previous = [];
                    if ($.isArray(new_arr))for (var i in new_arr)value_new[i] = $.extend(true, {}, new_arr[i]);
                    if ($.isArray(cur_arr))for (var i in cur_arr)value_previous[i] = $.extend(true, {}, cur_arr[i])
                }
                var eventData = obj.trigger({phase: "before", target: this.name, type: "change", value_new: value_new, value_previous: value_previous});
                if (eventData.isCancelled === true) {
                    $(this).val(obj.record[this.name]);
                    return false
                }
                var val = this.value;
        ...
        if (this.focus >= 0)setTimeout(focusEl, 500)
    }, destroy: function () {
        var eventData = this.trigger({phase: "before", target: this.name, type: "destroy"});
        if (eventData.isCancelled === true)return false;
        if (typeof this.toolbar == "object" && this.toolbar.destroy)this.toolbar.destroy();
